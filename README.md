...
Synapsis is an open-source project developed in the GitHub monorepo at [`dumitru-arsenii/synapsis`](https://github.com/dumitru-arsenii/synapsis). The npm packages in this workspace are published from that repository, so the source code, package docs, release pipeline, and issue tracker all live in one place.
...
- Packages: published to npm under the `@synapsis/*` scope
